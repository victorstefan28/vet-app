...
const RecordComponent: React.FC<RecordComponentProps> = ({ appointmentId }) => {
  ...
  const handleAddItem = async () => {
    // Implementation for adding items using the modal and ipc
    if (!expanded) return;
    ...
    <div>
      ...
      >
        <Box sx={modalStyle}>
          ...
          <FormControl fullWidth>
            <InputLabel id="works-select-label">Works</InputLabel>
            <Select
              sx={{ marginBottom: "20px" }}
          ...
          <FormControl fullWidth>
            <InputLabel id="labs-select-label">Labs</InputLabel>
          ...
          <ul>
            {toBeAdded.medications.map((med: RecordMedication, index) => (
              <>
                <li key={med.medication.id}>{med.medication.name}</li>
                <TextField
                  sx={{ marginTop: "10px" }}
                  value={med.quantity}
                  variant="standard"
